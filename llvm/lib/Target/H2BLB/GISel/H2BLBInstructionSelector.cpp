...
private:
  ...
  selectAddrMode(MachineOperand &Root) const;

  InstructionSelector::ComplexRendererFns
  selectSPAddrMode(MachineOperand &Root) const;
...
#include "H2BLBGenGlobalISel.inc"
#undef GET_GLOBALISEL_TEMPORAIRES_INIT
...
static void setRegisterClassForOperands(MachineInstr &I,
  ...
  for (MachineOperand &MO : I.operands()) {
    ...
    MRI.setRegClass(Reg,
                    Size == 16 ? &H2BLB::GPR16RegClass : &H2BLB::GPR32RegClass);
  }
}

bool H2BLBInstructionSelector::select(MachineInstr &MI) {
  unsigned Opcode = MI.getOpcode();
  if (isPreISelGenericOpcode(Opcode) && Opcode != ::TargetOpcode::PHI &&
  ...
  default:
    ...
    // override option outside of tablgen
  case TargetOpcode::G_PHI:
    MI.setDesc(TII.get(H2BLB::PHI));
    [[fallthrough]];
  case TargetOpcode::PHI:
  case TargetOpcode::COPY:
    setRegisterClassForOperands(MI, MRI);
    return true;
  ...
  case TargetOpcode::G_FRAME_INDEX:
    MI.setDesc(TII.get(H2BLB::MOVFROMSP));
...
H2BLBInstructionSelector::selectSPAddrMode(MachineOperand &Root) const {
  ...
  // instr -> block -> func -> data
  MachineRegisterInfo &MRI =
      Root.getParent()->getParent()->getParent()->getRegInfo();

  // get the instr that define the register
  MachineInstr *RootDef = MRI.getVRegDef(Root.getReg());
  if (RootDef->getOpcode() == TargetOpcode::G_FRAME_INDEX) {
