...
// CHECK-SAME: @"??R<lambda_1>@?0??test@@YAXXZ@QBE?A?<auto>@@UA@@@Z"
...
// CHECK-SAME: @"?__impl@<lambda_1>@?0??test@@YAXXZ@QBE?A?<auto>@@UA@@@Z"
// CHECK-SAME: (ptr noundef %this, ptr dead_on_return noundef %[[V]])

// CHECK: define internal noundef i32
// CHECK-SAME: @"?__invoke@<lambda_1>@?0??test@@YAXXZ@CA?A?<auto>@@UA@@@Z"
...
// CHECK: %{{.*}} = load i32, ptr @"?calls@?1???R<lambda_1>
// CHECK: %inc = add nsw i32 %{{.*}}, 1
// CHECK: store i32 %inc, ptr @"?calls@?1???R<lambda_1>
